...
class BmiPlatformSchema:
    ...
    # ── Azure context ───────────────────────────────────────────────────
    # Subscription used for deployment (RG, jumphost, BMI VMs).
    deployment_subscription_id: str = ""
    # Optional Azure ARM access token (JWT). When set, the deployer uses
    # it directly instead of ``DefaultAzureCredential`` to authenticate
    # to ARM. Pipelines wire this from ``S_LISA_azure_arm_access_token``
    # so we can run the BMI deploy inside docker without a host
    # ``az login`` session — same pattern as the lisav3 pipeline
    # (``lisa_connection`` service connection → token).
    azure_arm_access_token: str = ""
    # Region for the resource group and all resources.
    location: str = "southeastus5"
    ...
    # Jumphost admin password. Optional: when empty, the deployer
    # auto-generates an RSA SSH keypair and uses key-based auth
    # to bootstrap the iptables NAT rules. Plaintext password (when
    # supplied) is only used in-memory for the bootstrap connection.
    jumphost_password: str = ""
    # OpenSSH-format public key installed in the jumphost authorized_keys.
    # Auto-populated by the deployer when ``jumphost_password`` is empty;
    # may also be set explicitly to inject a caller-managed key.
    jumphost_public_key_data: str = ""
    # Path to the SSH private key matching ``jumphost_public_key_data``.
    # Auto-populated by the deployer when it generates the keypair.
